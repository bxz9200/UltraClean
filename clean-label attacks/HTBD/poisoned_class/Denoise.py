...
if know_poison_class == True:
    num_train = int((dataset_train.__len__() - dataset_poison.__len__())/2) + dataset_poison.__len__()
else:
...
for i in range(num_train):
    if know_poison_class == True:
        img, ground_truth = dataset_train.__getitem__(i+int(dataset_clean.__len__()/2))
    else:
...
if know_poison_class == True:
    index_color  =  distance_color_reduced_l1.argsort()[-int(threshold*len(dataset_poison)):][::-1]
    index_median =  distance_median_l1.argsort()[-int(threshold*len(dataset_poison)):][::-1]
    index_mean =  distance_mean_l1.argsort()[-int(threshold*len(dataset_poison)):][::-1]
    index_mix  =  distance_mix.argsort()[-int(threshold*len(dataset_poison)):][::-1]
...
if know_poison_class == True:
    index_color = index_color + int(dataset_clean.__len__()/2)
    index_median = index_median + int(dataset_clean.__len__()/2)
    index_mean = index_mean + int(dataset_clean.__len__()/2)
    index_mix = index_mix + int(dataset_clean.__len__()/2)
...
for index in index_mean:
#for index in index_median:
...
for i in range(len(training_img_path_from_txtfile)):
    if i not in del_clean_index:
        with open("./data/{}/retrain_finetune_filelist.txt".format(experimentID), 'a') as traintxt_w:
...
for i in range(len(poison_img_path_from_txtfile)):
    if i+len(dataset_clean) not in del_poison_index:
        with open("./data/{}/retrain_poison_filelist.txt".format(experimentID), 'a') as traintxt_w:
