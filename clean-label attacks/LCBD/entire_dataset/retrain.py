...
parser.add_argument('-d', '--detection_type', type=str, help='The type of detection (denoised, svd)')
parser.add_argument('-dr', '--detect_ratio', type=float, help='The detect ratio')
opts = parser.parse_args()
attack_type = opts.attack_type
detection_type = opts.detection_type
...
with open('config_retrain.json', 'r') as base_config_file:
...
poisoned_train_images = np.load("retrain_datasets/train_images_{}_{}_{}.npy".format(detection_type,attack_type,threshold_para)).astype(np.float32)
poisoned_train_labels = np.load("retrain_datasets/train_labels_{}_{}_{}.npy".format(detection_type,attack_type,threshold_para)).astype(np.int64)
...
shutil.copy('config_retrain.json', model_dir)
