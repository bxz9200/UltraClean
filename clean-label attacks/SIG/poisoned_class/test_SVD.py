...
state_dict = torch.load('retrain_cp_svd_{}/outputs/GTSRB/checkpoints/classifier.pt'.format(target_class), map_location='cuda:{}'.format(0))
...
with open("testlog/target_{}.log".format(target_class), 'a') as fid_w:
    fid_w.write("******************************************************* \n")
    fid_w.write("Testing(SVD) phase infomration:")
...
for kk in range(len(delta_var)):
    ...
    with open("testlog/target_{}.log".format(target_class), 'a') as fid_w:
        fid_w.write("\n After SVD detction, delta = {}, backdoor success rate = {:.2f}% \n".format(delta, 100*cnt/sum(num_img_test)))
...
print("Testing after SVD done, forward to denoise detection!!")