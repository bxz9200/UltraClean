...
state_dict = torch.load('checkpoints_mix_t1.5/checkpoint_{}/outputs/GTSRB/checkpoints/classifier.pt'.format(target_class), map_location='cuda:{}'.format(0))
...
if os.path.exists('./dataset/GTSRB-new/train-defensed_target_{}_{}_allclasses.txt'.format(target_class,threshold_para)):
    os.system('rm -r ./dataset/GTSRB-new/train-defensed_target_{}_{}_allclasses.txt'.format(target_class,threshold_para))
#     print("Done")
...
for i in range(len(training_img_path_from_txtfile)):
    ...
    if deleted_index not in index_mix:
        with open("./dataset/GTSRB-new/train-defensed_target_{}_{}_allclasses.txt".format(target_class,threshold_para), 'a') as traintxt_w:
